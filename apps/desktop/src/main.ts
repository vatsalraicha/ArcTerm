...
async function boot(mounts: Mounts): Promise<void> {
  ...
  const runAiShortcut = async (query: string, active: Session) => {
    if (!query) {
      // Bare "?" with nothing after: show a tiny hint and move on.
      active.terminal.writeRaw(
        "\r\n\x1b[2m(type ? followed by a description, e.g. `? find large files`)\x1b[0m\r\n",
      );
      editor.setValue("?");
      return;
    }
    // Clear the editor so there's no visible stale text while the
    // request is in flight. Also provide an inline status line —
    // written into the xterm buffer so it feels like a real operation.
    editor.clear();
    active.terminal.writeRaw(
      `\r\n\x1b[2mthinking about "${query}"…\x1b[0m`,
    );
    try {
      ...
      // Clear the "thinking…" line by rewriting over it + newline.
      active.terminal.writeRaw("\r\x1b[2K");
      if (!cmd) {
        active.terminal.writeRaw(
          "\x1b[33m(no command returned)\x1b[0m\r\n",
        );
        editor.setValue(`? ${query}`); // preserve the query for retry
        return;
      }
      // Show the generated command in the editor; user confirms by
      // pressing Enter (regular submit flow runs the non-? path) or
      // edits first.
      editor.setValue(cmd);
      editor.focus();
    } catch (err) {
      active.terminal.writeRaw(
        `\r\x1b[2K\x1b[31mAI error: ${err instanceof Error ? err.message : String(err)}\x1b[0m\r\n`,
      );
